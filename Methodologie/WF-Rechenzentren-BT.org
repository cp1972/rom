...
#+begin_src shell :results silent
  touch tangle.sh
  echo "#!/bin/bash" >> tangle.sh
  echo 'emacs --batch \' >> tangle.sh
  echo "      --eval \"(require 'org)\" \ " >> tangle.sh
  echo "      --eval \"(setq org-confirm-babel-evaluate nil)\" \ " >> tangle.sh
  echo "      --eval '(org-babel-tangle-file \"WF-Datenzentren-Bundestag.org\")' \ " >> tangle.sh
  chmod +x tangle.sh
#+end_src
...
#+name pdfpath
#+begin_src shell :results silent :var pdfpath="~/ROM-Data/ParlamentsDoc/Parlamentsdokumente_Sozialindex"
echo $pdfpath
#+end_src

#+name xmlpath
#+begin_src shell :results silent :var xmlpath="~/ROM-Data/XML-Quellen"
echo $xmlpath
#+end_src

#+begin_src shell :results silent :var xml=xmlpath :eval no
    cd ~
    cp -a ParlamentsDoc/. ParlamentsDoc_KOPIE/
    cp -a $xml/. XML-Quellen_KOPIE/
    mkdir Skripten-Rom
#+end_src
...
#+name skriptenpath
#+begin_src shell :results silent :var skriptenpath="~/RM-Data/Skripten-Rom"
echo $skriptenpath
#+end_src
...
#+begin_src shell :results silent :var skr=skriptenpath
  touch pdftxt.sh
  echo "#!/bin/bash" >> pdftxt.sh
  echo "for i in *.pdf; do pdftotext \"\$i\" ; done"  >> pdftxt.sh
  chmod +x pdftxt.sh
  mv pdftxt.sh $skr
#+end_src
...
#+name: umw_parl
#+begin_src shell :results silent :var parl=pdfpath :var skr=skriptenpath :eval no
  for dir in $parl; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and converting files" && bash $skr/pdftxt.sh && rm *.msg && rm *.docx; done;
#+end_src
...
#+begin_src shell :results silent :var skr=skriptenpath
  ...
  echo "words=( \"Rechenzentr\" )" >> filter.sh
  echo "for i in \"\${words[@]}\"; do grep \"\$i\" *.* | awk -F: '{print \$1}' | uniq > \"\$i\".txt && mkdir \"\$i\"_res && cp \$(<\"\$i\".txt) \"\$i\"_res && rm \"\$i\".txt; done" >> filter.sh
  chmod +x filter.sh
  mv filter.sh $skr
#+end_src
...
#+name: umw_parl2
#+begin_src shell :results silent :var parl=pdfpath :var skr=skriptenpath
  for dir in $parl; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and converting files" && bash $skr/filter.sh && rm *.msg && rm *.docx; done;
#+end_src
...
#+name: parser_prot
#+begin_src shell :results silent :var skr=skriptenpath :eval no
  touch parsprot.sh
  echo "#!/bin/bash" >> parsprot.sh
  echo "for i in *.xml; do sed -i 's/<sp /<div /' \"\$i\"; done" >> parsprot.sh
  echo "for i in *.xml; do pandoc -f html -t markdown -s \"\$i\" -o \${i%.*}.txt; done" >> parsprot.sh
  echo "rm *.xml" >> parsprot.sh
  echo "for i in *.txt; do year=\$(sed -n '3,5p' \"\$i\" | awk '\$8{print \$8}') && cp \"\$i\" \"\$year\"_\${i%.*}.fina; done" >> parsprot.sh
  echo "rm *.txt" >> parsprot.sh
  echo "for i in *.fina; do csplit --suppress-matched --prefix \${i%.*} --suffix-format _%03d.txt \"\$i\" '/type=/' '{*}'; done"  >> parsprot.sh
  echo "rm *.fina" >> parsprot.sh
  echo "rm *000_.txt" >> parsprot.sh
  echo "find . -type f -size 0b -delete" >> parsprot.sh
  chmod +x parsprot.sh
  mv parsprot.sh $skr
#+end_src
...
#+name: umw_prot
#+begin_src shell :results silent :var xml=xmlpath :var skr=skriptenpath :eval no
  for dir in $xml; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and converting files" && bash $skr/parsprot.sh; done;
#+end_src
...
#+begin_src shell :results silent :var xml=xmlpath :var skr=skriptenpath
  for dir in $xml; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and converting files" && bash $skr/filter.sh; done;
#+end_src
...
#+name: zerleg_prot
#+begin_src shell :results silent :var skr=skriptenpath
  touch zerleg-prot.sh
  echo "#!/bin/bash" >> zerleg-prot.sh
  echo "for i in *.txt; do csplit --prefix \${i%.*} --suffix-format _%04d.finc \"\$i\" '/who=/' '{*}'; done" >> zerleg-prot.sh
  echo "rm *.txt" >> zerleg-prot.sh
  echo "for i in *.finc; do nnp=\$(grep \"{who=\" \"$i\" | sed 's/\" /@/g' | awk -F\"@\" '{print \$1\"-\"\$3\"-\"\$6}' | sed -e 's/\"//g' -e 's/{//g' -e 's/=//g' -e 's/who//g' -e 's/role//g' -e 's/party//g' -e 's/^::: //g' | sed 's/ //g') && cp \"\$i\" \${i%.*}_\"\$nnp\".txt ; done" >> zerleg-prot.sh
  echo "rm *.finc" >> zerleg-prot.sh
  echo "for i in *.txt; do sed -i '/^::: /d' \"\$i\"; done" >> zerleg-prot.sh
  echo "find . -type f -size 0b -delete" >> zerleg-prot.sh
  chmod +x zerleg-prot.sh
  mv zerleg-prot.sh $skr
#+end_src
...
#+begin_src shell :results silent :var xml=xmlpath :var skr=skriptenpath
for dir in $xml/01; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/02; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/03; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/04; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/05; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/06; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/07; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/08; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/09; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/10; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/11; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/12; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/13; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/14; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/15; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/16; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/17; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
for dir in $xml/18; do [ -d "$dir" ] && cd "$dir" && echo "Entering into $dir and parsing files" && sh $skr/zerleg-prot.sh; done;
#+end_src
