...
#+begin_src shell :results output
  touch meth-bundest.sh
  echo "#!/bin/bash" >> meth-bundest.sh
  echo 'emacs --batch \' >> meth-bundest.sh
  echo "      --eval \"(require 'org)\" \ " >> meth-bundest.sh
  echo "      --eval \"(setq org-confirm-babel-evaluate nil)\" \ " >> meth-bundest.sh
  echo "      --eval '(org-babel-tangle-file \"WF-Datenzentren-Bundestag.org\")' \ " >> meth-bundest.sh
  chmod +x meth-bundest.sh
#+end_src

#+RESULTS:

Dieser Skript führt alle Code-Blöcke aus, die sich in diesem Dokument befindet. Er wird jedoch die im Rahmen von diesen Blöcken hergestellten Skript nicht ausführen. Wenn das erwünscht ist, dann sollten die Skripts zu meth-bundest.sh hinzugefügt werden (etwa: =sh script.sh=).

* Parlamentsdokumente

** Vorbereitung und Umwandlung

Wir gehen davon aus, dass die Ordner mit den Parlamentsdokumenten und Protokollen im Benutzerpfad gespeichert wurden.
Im Benutzerpfad stellen wir einen Ordner "Results" und einen Ordner "Skripten" her, um unsere Ergebnisse und unsere Skripten zu speichern.

#+name: benutzer_pfad
#+begin_src shell :results silent
  cd ~ && mkdir Results && mkdir Skripten
#+end_src

Wir machen eine Sicherheitskopie vom Ordner ParlamentsDoc und vom Ordner XML-Quellen.
...
Wir wandeln die *.pdf-Dateien der Parlamentsdokumente mit folgendem Skript, den wir im Ordner "Skripten" speichern.
...
Wir brauchen einen Skript, der uns zu jedem Ordner bringt, wo wir die Dokumente umwandeln und andere Dateien löschen, die wir für die Analyse nicht brauchen.

#+name: umw_parl
#+begin_src shell
  touch enterdir.sh
  echo "#!/bin/bash"
  echo "userpath='~/ParlamentsDoc/Parlamentsdokumente_Sozialindex/'" >> enterdir.sh
  echo "parserpath='~/Skripten/'" >> enterdir.sh
  echo "for dir in $userpath; do [ -d \"$dir\" ] && cd \"$dir\" && echo \"Entering into $dir and converting files\" && bash $parserpath/pdftxt.sh && rm *.msg && rm *.docx; done;" >> enterdir.sh
  chmod +x enterdir.sh
#+end_src

Wir können jetzt die Umwandlung vornehmen.

#+begin_src shell :results silent
    mv pdftxt.sh ~/Skripten
    mv enterdir.sh ~/Skripten
    cd ~/Skripten
    sh enterdir.sh
#+end_src
