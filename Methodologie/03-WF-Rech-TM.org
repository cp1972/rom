...
Ähnlich wie bei unserem 'tesi' Code-Block bereiten wir einen Code-Block für die automatische Durchführung des SCT.py-Skript. Im Unterschied zu 'tesi' Code-Block brauchen wir keine Variable für den Pfad zu den Stopwords zu definieren, da unser Skript keine Stopword benutzt. Wir definieren Wörter und Wörterwurzel, die wir den SCT.py-Skript senden, damit er die Sätze in den Berichten zu den Topics aufnimmt, die diese Wörter und entsprechende Wörter mit dem gegebenen Wurzel enthalten. Wir geben weiter die minimale Anzahl an Sätzen je Cluster, die wir bilden wollen (hier 2, weil wir wenige Dokumente haben), und wir geben das Koeffizient zwischen 0.0 und 0.99 für die Verbindung zwischen Sätzen an. Je höher dieses Koeffizient ist, desto enger sind die Sätzen inhaltlich verbunden. Da wir im Ziel 1 gesehen haben, dass das Thema Rechenzentren wahrscheinlich wenig in die Debatten des Bundestages auftaucht, die wir modelliert haben, wählen wir ein Koeffizient von 0.4 für eine schwache jedoch nicht zufällige inhaltliche Verbindung zwischen Sätzen, die die Cluster bilden.

Wir können den Code-Block 'sct' mit diesen Informationen herstellen:
...
#+begin_src shell :var mtaki="/home/cpsoz/ROM-Data/TM-Analyse/MTA-KI" :var user=nuser :var stopw=stops  :results none
  ...
  cat $user/BERT-Skripten/sct.txt | python3 $user/BERT-Skripten/TeSi.py
#+end_src
