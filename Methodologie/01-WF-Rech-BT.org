...
Dieses Dokument beschreibt ein /workflow/ zur Behandlung der Parlamentsdokumenten und Protokollen des Bundestages (1949-2025). Es wandelt diese Dokumente in Text-Dateien um, und es benennt sie in mehreren Schritten um, sortiert und zerlegt sie. Dieses /workflow/ läuft unter einem Linux-Betriebssystem -- wir haben es mit dem Linux-Kernel 5.4 getestet -- und bezieht die Anwendungen, die diese Familie von Betriebssystemen anbietet. Entsprechende Anpassungen in diesem Dokument müssen unter Windows und MacOS vorgenommen werden. Für Windows-Benutzer wird die Installation von WSL2 empfohlen, das erlaubt, auf Linux zuzugreifen und die Anwendungen zu verwenden, die in diesem /workflow/ verwendet werden.

Die Parlamentsdokumente der Bundesrepublik Deutschland und Protokollen des Bundestages können an der folgenden Adresse als xml-Dateien, mit einer API oder nach handwerklicher Suche heruntergeladen werden: https://www.bundestag.de/services/opendata.

Ein Github-Konto bietet ebenfalls ein R-Objekt an: https://github.com/benjaminguinaudeau/tidybundestag.
...
Inhaltlich liefern diese Dokumente unterschiedliche Informationen zu den politischen Entscheidungen, die von 1949 bis heute (Juni 2025) auf Bundesebene getroffen werden. Bezogen auf das Thema des Vorhabens in dieser Arbeit erlauben sie eine Rekonstruktion des thematischen Zusammenhanges, in dem die Rechenzentren in Deutschland diskutiert wurden.
...
Pfade müssen von Benutzern des Workflows angepasst werden -- statt /home/cpsoz/ROM-Data muss ein Benutzer seinen eigenen Pfad zu den Dokumenten schreiben.

Wir schreiben den Inhalt vom Skript 'datab-rech.sh' und wir machen den Skript ausführbar (Pfade anpassen):
...
Dieser Skript führt alle Code-Blöcke in diesem /workflow/ aus. Er kann entsprechend mit bash verwendet werden, etwa: bash datab-rech.sh oder dann ./datab-rech.sh.

WICHTIG!!!: Damit Pfade korrekt erkannt werden, müssen alle ':results' in Code-Blöcken deaktiviert werden, also ':results silent'

Wir fangen jetzt mit der Behandlung der Parlamentsdokumente an.

* Parlamentsdokumente

** Vorbereitung und Umwandlung

Wir machen eine Sicherheitskopie vom Ordner 'ParlamentsDoc' und vom Ordner 'XML-Quellen', in denen wir die Parlamentsdokumente und die Protokollen des Bundestages gespeichert haben. Wir stellen einen Ordner 'Skripten-Rom' für die Skripten her, die wir im Laufe dieses /workflows/ generieren werden.

Wir definieren die Pfade zu den PDF- und XML-Dateien (Pfade anpassen).
...
Wir machen eine Kopie der Quelldateien (Pfade anpassen).
...
Wir behalten den Pfad zum Skripten-Ordner in einer Variable 'skriptenpath' (Pfade anpassen).
...
Wir sortieren die konvertierten Dateien nach dem Stichwort "Rechenzentr", um alle Varianten vom Begriff "Rechenzentrum" (Nominativ, Genitiv, Plural/Singular) abzufangen. Dafür brauchen wir einen Skript, der in alle Ordner geht, in denen die Dokumente gespeichert sind, und nur die Dokumente aufnimmt, die eine Variante vom Stichwort enthalten. Diese Dokumente werden in einem Ordner "Rechenzentr_res" gespeichert.
...
Wir müssen die Dateien in den jeweiligen Unterordnern (von 01 bis 19) konvertieren. Es sind XML-Dateien, die wir mit Pandoc konvertieren. Dabei wollen wir zwei Zeilen (<div> und <sp> Zeilen) aus den Quellen behalten, weil diese Zeilen Metadaten enthalten, die wir benutzen werden, um die Dateien in einem späteren Schritt zu zerlegen.

** Vorbereitung und Umwandlung

Wir löschen den Ordner 19 in 'XML-Quellen', der anders als die anderen formatiert ist. Wir behandeln diesen Ordner mit einer veränderten Version von parsprot.sh und zerlg-prot.sh später (s.u.).
...
Wir haben jetzt eine Datei je Sitzung und TOP. Wir müssen diese Dateien nach dem für uns relevanten Stichwort "Rechenzentr" sortieren, bevor wir sie nach Sprechern in den jeweiligen TOP zerlegen. Wir sortieren zuerst die Dateien mit dem gleichen Skript wie für die Parlamentsdokumente.
...
Dann können wir die Protokolle nach Sprechern zerlegen sowie die Dateien nach Sprecher/Rolle/Partei umbenennen. Wir brauchen einen neuen Skript für die Zerlegung der Dateien.
...
Wir stellen einen Ordner 'XML-Results' her, und wir kopieren alle Dateien aus den Unterordnern 'Rechenzentr_res' in diesem Ordner:
...
Damit haben wir alle relevante Protokolle nach Sprecher, Rolle und Partei zerlegt. 6408 Dateien werden generiert, die wir später mit den anderen Dateien nach Wahlperioden in einem gemeinsamen Ordner zusammenstellen werden. Wir müssen noch den Ordner 19 aus den XML-Quellen behandeln, der Dateien enthält, die anders als die Dateien in den anderen Ordnern formatiert wurden.
...
Wir schaffen einen Ordner '19-Results' im Arbeitsverzeichnis, und wir kopieren alle Dateien aus den Unterordnern 'Rechenzentr_res' zu diesem Ordner:
...
Wir brauchen anschließend eine veränderte Version von unserem Skript 'zerleg-prot.sh', da die Metadaten in den Protokollen des Ordners 19 unterschiedlich plaziert sind:
...
Wir wandeln die *.xml-Dateien in *.txt-Dateien um:
...
Unsere Dateien befinden sich in drei unterschiedlichen Ordnern, weshalb wir sie in einem gemeinsamen Ordner speichern möchten. Dafür schaffen wir den gemeinsamen Ordner 'Rechenzentren' mit folgenden Zeilen:
...
Wir kopieren die Dateien von den drei 'Results' Ordnern zu unserem Rechenzentren-Ordner.
...
Unser Datensatz ist jetzt bereit für die Analyse. Diese erfolgt im einem [[file+sys:~/Github/rom/Methodologie/02-WF-Rech-TM.org][zweiten Workflow]].
